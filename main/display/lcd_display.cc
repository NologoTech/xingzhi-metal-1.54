...
SpiLcdDisplay::SpiLcdDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel,
    ...
    port_cfg.task_priority = 1;//1 //7
    port_cfg.timer_period_ms = 50;//50  //5
...
void LcdDisplay::SetupUI() {
    ...
    lv_obj_set_style_bg_color(container_, current_theme_.background, 0);
    lv_obj_set_style_border_color(container_, current_theme_.border, 0);
    lv_obj_set_scrollbar_mode(container_, LV_SCROLLBAR_MODE_OFF);//jin'du'tiao

    gif_emoji_label_ = lv_gif_create(container_);
    lv_obj_move_background(gif_emoji_label_);

    lv_obj_set_style_bg_opa(gif_emoji_label_, LV_OPA_TRANSP,  0);  // 背景完全透明
    lv_obj_set_style_border_opa(gif_emoji_label_, LV_OPA_TRANSP, LV_PART_MAIN);  // 无边框
    lv_obj_set_style_pad_all(gif_emoji_label_, 0, LV_PART_MAIN);  // 去除内边距
    lv_obj_align(gif_emoji_label_, LV_ALIGN_CENTER, 0, 0);
    LV_IMAGE_DECLARE(loading);
    lv_gif_set_src(gif_emoji_label_, &loading);

    network_label_ = lv_img_create(screen);
    LV_IMAGE_DECLARE(xh5);
    lv_img_set_src(network_label_, &xh5);
    // lv_obj_set_pos(network_label_, 6, 6);
    lv_obj_align(network_label_, LV_ALIGN_TOP_LEFT, 0, 0);
    // lv_img_set_src(battery_label_, &dc5);
    battery_label_ = lv_img_create(screen);
    lv_img_set_src(battery_label_, NULL); // 设置为空
    lv_obj_align(battery_label_, LV_ALIGN_TOP_RIGHT, 0, 5);

    net_label_ = lv_img_create(screen);
    lv_img_set_src(net_label_, NULL); // 设置为空
    lv_obj_align(net_label_, LV_ALIGN_TOP_RIGHT, -(30 + 2), 4); 
    // lv_obj_align(net_label_, LV_ALIGN_TOP_RIGHT, 0, 0);
    ...
    // lv_label_set_long_mode(chat_message_label_, LV_LABEL_LONG_SCROLL_CIRCULAR); // 循环滚动（推荐）
    // lv_obj_align_to(chat_message_label_, screen, LV_ALIGN_TOP_MID, 0, 32);  //32
    // lv_obj_set_style_text_align(chat_message_label_, LV_TEXT_ALIGN_CENTER, 0); // 设置文本居中对齐
    // lv_obj_set_y(chat_message_label_, 16);
    // lv_obj_set_style_border_width(chat_message_label_, 2, 0); // 边框宽度为2px
    // lv_obj_set_style_border_color(chat_message_label_, lv_color_hex(0xAAAAAA), 0); // 边框颜色为灰色
    // lv_obj_set_style_border_opa(chat_message_label_, LV_OPA_100, 0); // 边框完全不透明
    // lv_obj_set_style_radius(chat_message_label_, 4, 0); // 可选：添加圆角使边框更美观
    // lv_obj_set_style_pad_all(chat_message_label_, 8, 0); // 可选：添加内边距，使文本不紧贴边框
    // lv_obj_set_style_bg_color(chat_message_label_, lv_color_hex(0xFFFFFF), 0); // 白色背景
    // lv_obj_set_style_bg_opa(chat_message_label_, LV_OPA_100, 0); // 背景完全不透明
    // lv_obj_set_style_text_color(chat_message_label_, lv_color_hex(0x000000), 0); // 黑色文本


    chat_message_label_ = lv_label_create(screen);
    lv_obj_remove_style_all(chat_message_label_);
    lv_obj_set_scrollbar_mode(chat_message_label_, LV_SCROLLBAR_MODE_OFF);
    lv_label_set_text(chat_message_label_, "");
    lv_obj_set_style_radius(chat_message_label_, 12, 0);
    lv_obj_set_style_bg_color(chat_message_label_, lv_color_hex(0xFFFFFF), 0);
    lv_obj_set_style_bg_opa(chat_message_label_, LV_OPA_90, 0);
    lv_obj_set_style_border_width(chat_message_label_, 1, 0);
    lv_obj_set_style_border_color(chat_message_label_, lv_color_hex(0xF5F5F5), 0);
    lv_obj_set_style_pad_all(chat_message_label_, 10, 0);
    ...
    // lv_obj_add_flag(notification_label_, LV_OBJ_FLAG_HIDDEN);
    notification_label_ = lv_label_create(screen);
    lv_obj_set_style_text_font(notification_label_, &font_puhui_16_4, 0);
    lv_label_set_text(notification_label_, "");
    ...
    // lv_obj_set_pos(notification_label_, 0, 6);
    ...
    lv_obj_align(mute_label_, LV_ALIGN_TOP_MID, 0, 6);
    lv_obj_set_pos(mute_label_, 0, 6);
...
void LcdDisplay::SetEmotion(const char* emotion) {
    ...
    DisplayLockGuard lock(this);
    if (gif_emoji_label_ == nullptr) {
        return;
    }
    // 根据表情设置对应的 GIF
    if (it != emotions.end()) {
        if (strcmp(it->text, "neutral") == 0) {
            LV_IMAGE_DECLARE(nature);
            lv_gif_set_src(gif_emoji_label_, &nature);
        } else if (strcmp(it->text, "happy") == 0) {
            LV_IMAGE_DECLARE(nature);
            lv_gif_set_src(gif_emoji_label_, &nature);
        } else if (strcmp(it->text, "laughing") == 0) {
            LV_IMAGE_DECLARE(nature);
            lv_gif_set_src(gif_emoji_label_, &nature);
        } else if (strcmp(it->text, "funny") == 0) {
            LV_IMAGE_DECLARE(nature);
            lv_gif_set_src(gif_emoji_label_, &nature);
        } else if (strcmp(it->text, "sad") == 0) {
            LV_IMAGE_DECLARE(nature);
            lv_gif_set_src(gif_emoji_label_, &nature);
        } else if (strcmp(it->text, "angry") == 0) {
            LV_IMAGE_DECLARE(nature);
            lv_gif_set_src(gif_emoji_label_, &nature);
        } else if (strcmp(it->text, "crying") == 0) {
            LV_IMAGE_DECLARE(nature);
            lv_gif_set_src(gif_emoji_label_, &nature);
        } else if (strcmp(it->text, "loving") == 0) {
            LV_IMAGE_DECLARE(nature);
            lv_gif_set_src(gif_emoji_label_, &nature);
        } else if (strcmp(it->text, "embarrassed") == 0) {
            LV_IMAGE_DECLARE(nature);
            lv_gif_set_src(gif_emoji_label_, &nature);
        } else if (strcmp(it->text, "surprised") == 0) {
            LV_IMAGE_DECLARE(nature);
            lv_gif_set_src(gif_emoji_label_, &nature);
        } else if (strcmp(it->text, "shocked") == 0) {
            LV_IMAGE_DECLARE(nature);
            lv_gif_set_src(gif_emoji_label_, &nature);
        } else if (strcmp(it->text, "thinking") == 0) {
            LV_IMAGE_DECLARE(thinking);
            lv_gif_set_src(gif_emoji_label_, &thinking);
        } else if (strcmp(it->text, "winking") == 0) {
            LV_IMAGE_DECLARE(nature);
            lv_gif_set_src(gif_emoji_label_, &nature);
        } else if (strcmp(it->text, "cool") == 0) {
            LV_IMAGE_DECLARE(nature);
            lv_gif_set_src(gif_emoji_label_, &nature);
        } else if (strcmp(it->text, "relaxed") == 0) {
            LV_IMAGE_DECLARE(nature);
            lv_gif_set_src(gif_emoji_label_, &nature);
        } else if (strcmp(it->text, "delicious") == 0) {
            LV_IMAGE_DECLARE(nature);
            lv_gif_set_src(gif_emoji_label_, &nature);
        } else if (strcmp(it->text, "kissy") == 0) {
            LV_IMAGE_DECLARE(nature);
            lv_gif_set_src(gif_emoji_label_, &nature);
        } else if (strcmp(it->text, "confident") == 0) {
            LV_IMAGE_DECLARE(nature);
            lv_gif_set_src(gif_emoji_label_, &nature);
        } else if (strcmp(it->text, "sleepy") == 0) {
            LV_IMAGE_DECLARE(sleeping);
            lv_gif_set_src(gif_emoji_label_, &sleeping);
        } else if (strcmp(it->text, "silly") == 0) {
            LV_IMAGE_DECLARE(nature);
            lv_gif_set_src(gif_emoji_label_, &nature);
        } else if (strcmp(it->text, "confused") == 0) {
            LV_IMAGE_DECLARE(thinking);
            lv_gif_set_src(gif_emoji_label_, &thinking);
        }else if (strcmp(it->text, "loading") == 0) {
            LV_IMAGE_DECLARE(loading);
            lv_gif_set_src(gif_emoji_label_, &loading);
        }else if (strcmp(it->text, "uping") == 0) {
            lv_obj_add_flag(gif_emoji_label_, LV_OBJ_FLAG_HIDDEN);
        }
    }
} 
#endif
