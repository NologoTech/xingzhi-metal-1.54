...
void Cst816x::touchpad_daemon(void* arg) {
    ...
    auto& dual_network_board = static_cast<DualNetworkBoard&>(Board::GetInstance());  
    ...
    while (1) {
        ...
        if (tp.num > 0 && !cst816x->is_touching_) {
            ...
            current_event = {TouchEventType::PRESS_CLICK, tp.x, tp.y};
            event_detected = true;
        }
        ...
        if (event_detected) {
            if (current_event.y == 600 && (current_event.x == 20 || current_event.x == 40 || current_event.x == 60)) {
                switch (current_event.type) {
                    case TouchEventType::PRESS_CLICK:
                        gpio_set_level(ZHENDONG_PIN, 1);
                        vTaskDelay(pdMS_TO_TICKS(50));
                        gpio_set_level(ZHENDONG_PIN, 0);
                        break;
                        ...
                        if (current_event.x == 40) {
                            ...
                            if (dual_network_board.GetNetworkType() == NetworkType::WIFI) {
                                if (app.GetDeviceState() == kDeviceStateStarting) {
                                    // cast to WifiBoard
                                    auto& wifi_board = static_cast<WifiBoard&>(dual_network_board.GetCurrentBoard());
