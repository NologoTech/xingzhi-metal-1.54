...
// #include "esp_blufi.h"
#include "esp_efuse.h" //读序列号

#include "application.h"//播放音效
...
/**后加变量 */
static BLUFI_WIFI blifi_wifi_;
static bool ssid_sta=false;
static bool passd_sta=false;
static bool connect_blufi=false;
                                ...
                                int32_t event_id, void* event_data)
{
    ...
    case IP_EVENT_STA_GOT_IP: {
        ...
        ssid_manager.AddSsid(blifi_wifi_.blufi_ssid, blifi_wifi_.blufi_passd);
...
static void example_event_callback(esp_blufi_cb_event_t event, esp_blufi_cb_param_t *param)
{
    ...
    case ESP_BLUFI_EVENT_BLE_DISCONNECT:
        ...
        esp_blufi_adv_start();
        break;
    case ESP_BLUFI_EVENT_SET_WIFI_OPMODE://pei1
        BLUFI_INFO("ESP_BLUFI_EVENT_SET_WIFI_OPMODE\n");
        break;
    case ESP_BLUFI_EVENT_REQ_CONNECT_TO_AP://pei4
	...
	case ESP_BLUFI_EVENT_RECV_STA_BSSID:
        ...
        break;
	case ESP_BLUFI_EVENT_RECV_STA_SSID://pei2
        BLUFI_INFO("ESP_BLUFI_EVENT_RECV_STA_SSID=%s\n",(char *)param->sta_ssid.ssid);
        ...
        /*加入wifi */
        blifi_wifi_.blufi_ssid.assign(
            reinterpret_cast<const char*>(param->sta_ssid.ssid),
            param->sta_ssid.ssid_len
        );
        // esp_wifi_set_config(WIFI_IF_STA, &sta_config);
        BLUFI_INFO("Recv STA SSID %s\n", blifi_wifi_.blufi_ssid.c_str());
        ssid_sta=true;
        break;
	case ESP_BLUFI_EVENT_RECV_STA_PASSWD://pei3
        BLUFI_INFO("ESP_BLUFI_EVENT_RECV_STA_PASSWD:%s\n",(char *)param->sta_passwd.passwd);
    ...
    case ESP_BLUFI_EVENT_RECV_CUSTOM_DATA:{
        ...
        sprintf(rec_s, "%s", param->custom_data.data);
        BLUFI_INFO("---rec_s----=%s",rec_s);
...
{
    auto& app = Application::GetInstance();//yin
    ...
    while (1)
    {
        if (connect_blufi==true)
        {
            uint8_t serial_number[33] = {0}; // 假设数据长度为32字节
            size_t size = 32;
            esp_err_t ret = esp_efuse_read_block(EFUSE_BLK3, serial_number, 0, size * 8);
            bool is_empty = false;
            if (ret != ESP_OK) {
                is_empty = true;
            } else {
                if (strlen(reinterpret_cast<const char*>(serial_number))==0)
                {
                    is_empty = true;
                }
            }
            vTaskDelay(pdMS_TO_TICKS(1000));
            if (ret == ESP_OK && !is_empty) {
                serial_number[size] = '\0'; // 确保字符串以\0结尾
                client_id = std::string(" {\"serial_number\":\"")+reinterpret_cast<const char*>(serial_number)+std::string("\"}");
                esp_blufi_send_custom_data((uint8_t *)client_id.c_str(),client_id.size());
                BLUFI_INFO("BLUFI VERSION %s,%d\n", client_id.c_str(),client_id.size());
            }else{
                char blu_mac[36] = {0};
                uint8_t sta_mac[6] = {0};
                esp_base_mac_addr_get(sta_mac);
                sprintf(blu_mac, "{\"serial_number\":\"%02X%02X%02X%02X%02X%02X\"}", sta_mac[0],sta_mac[1],sta_mac[2],sta_mac[3],sta_mac[4],sta_mac[5]);
                esp_blufi_send_custom_data((uint8_t *)blu_mac,strlen(blu_mac));
            }
            connect_blufi=false;
        }

        if (ssid_sta==true && passd_sta==true)
        {
            auto& ssid_manager = SsidManager::GetInstance();
            ssid_manager.AddSsid(blifi_wifi_.blufi_ssid, blifi_wifi_.blufi_passd);
            vTaskDelay(pdMS_TO_TICKS(1000));
        ...
        // BLUFI_INFO("Largest free block: %d bytes", max_block);
        // BLUFI_INFO( "Blufi Free internal: %u minimal internal: %u", free_sram, min_free_sram);
    }
}