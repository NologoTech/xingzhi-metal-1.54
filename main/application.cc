...
void Application::ShowActivationCode(const std::string& code, const std::string& message) {
    ...
    for (size_t i = 0; i < modified.size(); ++i) {
        if (modified[i] == '\r' || modified[i] == '\n') {
            modified[write_pos++] = ' ';
        } else {
            modified[write_pos++] = modified[i];
        }
    }
    modified.resize(write_pos);  // 恢复 resize，确保字符串长度正确（否则可能包含无效字符）

    // 移除 "xiaozhi.me" 部分
    std::string to_remove = "xiaozhi.me";
    size_t pos = modified.find(to_remove);  // 查找 "xiaozhi.me" 的位置
    if (pos != std::string::npos) {  // 如果找到
        // 计算需要保留的起始位置（跳过 "xiaozhi.me" 及其后的空格）
        size_t start = pos + to_remove.length();
        // 跳过 "xiaozhi.me" 后面的空格（如果有的话）
        while (start < modified.size() && modified[start] == ' ') {
            start++;
        }
        // 截取从 start 到结尾的部分（即去掉前面的 "xiaozhi.me"）
        modified = modified.substr(start);
    }

    // 调用 Alert 时，modified 已去掉 "xiaozhi.me"
    Alert(Lang::Strings::ACTIVATION, modified.c_str(), "happy", Lang::Sounds::P3_ACTIVATION);
    for (const auto& digit : code) {
        auto it = std::find_if(digit_sounds.begin(), digit_sounds.end(),
            [digit](const digit_sound& ds) { return ds.digit == digit; });
        if (it != digit_sounds.end()) {
